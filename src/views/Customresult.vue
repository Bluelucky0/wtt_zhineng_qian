<template>
  <div>
    <div class="contence">
      <p class="home"><a href="#/header/main">首页</a>><span>专属推荐--结果</span></p>
      <div class="allcourses">
        <div class="recomment">
          <ul class="recomeEach"
              id="recomeEach">
            <li v-for="(item,index) in jingqus"
                :class="{'on':item.showscal}"
                :key="item.id">
              <div class="coursecover">
                <img :src="item.cover"
                     alt=""
                     onerror="this.src='../assets/image/hot.jpg'">
              </div>
...
export default {
  ...
  mounted () {
    this.jingqus = JSON.parse(this.$route.params.customviews)
    if (this.jingqus.length == 0) {
      this.hasnoresult = true
    } else {
      this.hasnoresult = false
    }
    console.log(this.jingqus)
  },
}
</script>
